...
Promoter_motif_ggplot_NoPi <- function(Annotation_data = S288cAnnotation_sacCer1, motif = sacCer1Motif, motif_rev = sacCer1REVMotif, gene = "YBR093C"){
  ...
  if (line$strand == "-"){
    ...
    Promoter_ggplot_NoPi(data = Annotation_data, gene = gene) + geom_segment(aes(x = as.numeric(start), y = 0, xend = as.numeric(end), yend = 0), 
                                        data = motifs, size = 5, col = ifelse(motifs$motif=="CACGTG", "red","blue"))
  }else{
    motifs <- motif[which(motif$chromosome == line$chromosome & as.numeric(motif$start)
                               >= line$start - 800 & as.numeric(motif$end) <= line$start),]
    Promoter_ggplot_NoPi(data = Annotation_data, gene = gene) + geom_segment(aes(x = as.numeric(start), y = 0, xend = as.numeric(end), yend = 0), 
                                     data = motifs_rev, size = 5, col = ifelse(motifs$motif=="GTGCAC", "hotpink","cyan"))
  }()
}
